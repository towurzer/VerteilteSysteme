...
import static org.aau.homework.assignment_09.ex1.ChordSystem.isInCircularInterval;
import static org.aau.homework.assignment_09.ex1.ChordSystem.isInCircularIntervalExclusiveUpperBound;
...
public class Node {
    ...
    public Node searchForKey(int key){
        System.out.printf("Node %d is searching for %d. (Finger Table: '%s')%n", id, key, getFingerTableAsString());
    ...
    public String getFingerTableAsString() {
        ...
        for (int i = 0; i < m; i++) {
            sb.append(fingerTable[i] != null ? fingerTable[i].id : "null");
            if (i < m - 1) {
                sb.append(", ");
            }
        }
